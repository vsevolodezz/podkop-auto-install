...
update_sing_box() {
	...
	printf "\033[32;1m[*] Searching for latest version $FILE_PATTERN...\033[0m\n"
	...
	if [ -z "$DOWNLOAD_URL" ]; then
		...
		echo "Check $FILE_PATTERN or repository."
		exit 1
	fi

	printf "\033[32;1m[+] Found: $DOWNLOAD_URL\033[0m\n"

	printf "\033[32;1m[*] Preparing temporary directory...\033[0m\n"
	rm -rf "$TMP_DIR"
	mkdir -p "$TMP_DIR"
	cd "$TMP_DIR"

	printf "\033[32;1m[*] Downloading...\033[0m\n"
	...
	printf "\033[32;1m[*] Stopping old sing-box...\033[0m\n"
	service sing-box stop >/dev/null 2>&1 || true
	killall sing-box >/dev/null 2>&1 || true

	printf "\033[32;1m[*] Extracting archive...\033[0m\n"
	...
	printf "\033[32;1m[*] Searching for binary file...\033[0m\n"
	BINARY_PATH=$(find . -type f -name sing-box | head -n 1)

	if [ -z "$BINARY_PATH" ]; then
		echo "[!] ERROR: Archive extracted but no 'sing-box' binary found!"
		exit 1
	fi

	printf "\033[32;1m[+] Binary found: $BINARY_PATH\033[0m\n"

	printf "\033[32;1m[*] Installing new binary to $DEST_FILE...\033[0m\n"
	mv "$BINARY_PATH" "$DEST_FILE"

	printf "\033[32;1m[*] Setting execute permissions...\033[0m\n"
	chmod +x "$DEST_FILE"

	printf "\033[32;1m[*] Cleaning up...\033[0m\n"
	cd /
	rm -rf "$TMP_DIR"

	printf "\033[32;1m[+] Sing-box updated successfully!\033[0m\n"
}
...
printf "\033[32;1mUpdating sing-box...\033[0m\n"
update_sing_box
